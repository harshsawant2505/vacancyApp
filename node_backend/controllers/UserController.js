...
async function registerPolice(req,res) {
    ...
    try {
        ...
        const saltRounds = 10; // You can adjust this value
        const hashedPassword = await bcrypt.hash(password, saltRounds);
        console.log(name, email, numberPlate, phNo,  hashedPassword, policeEmail); 
        
        req.session.user = {name, email, numberPlate, phNo, password: hashedPassword, policeEmail};
        ...
        const user = new User({name, email, numberPlate, phNo, password: hashedPassword, policeEmail});
